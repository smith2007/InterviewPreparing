...
public class TextJustification {
   ...
   * "shall be    " instead of "shall     be", because the last line must be left-justified instead
   * of fully-justified. Note that the second line is also left-justified becase it contains only
   * one word. Example 3:
   * <p>
   ...
   * is  what we", "understand      well", "enough to explain to", "a  computer.  Art is",
  ...
  private int findRight(int left, String[] words, int maxWidth) {
    ...
    //пока не дойдем до конца массива words или общая сумма длинн не будет больше чем максимальная
    while (right < words.length && (sum + 1 + words[right].length()) <= maxWidth) {
      sum += 1 + words[right].length(); //+1 делаем изза теоретического пробела
      right++;
    }
