...
public class AddTwoNumbersII {
    ...
    /**
     * короче надо сделать сложение с начала, не получится просто пройтись с конца
     *  разворачивать линкед листы нельзя
     *
     * Input: (7 -> 2 -> 4 -> 3) + (5 -> 6 -> 4)
     * Output: 7 -> 8 -> 0 -> 7
     *
     * тут принцип выпячивания назад  через два стека - накидали в два стека данные из двух списков
     * сначал построили ноду currNode, засетили ее в ноль
     *
     * тут делается очень своеобразный трюк - мы строим новый линкед лист как бы пятясь назад и пристраивая к нему newHead
     * крутимся в цикле пока оба стека не опустеют - while (!s1.empty() || !s2.empty())
     *
